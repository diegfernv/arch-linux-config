...
function getNickname() {
    ...
    if [ -n "$nodeName" ] && [ -n "$portName" ] && [ -n "${SINK_NICKNAMES[$nodeName/$portName]}" ]; then
        SINK_NICKNAME="${SINK_NICKNAMES[$nodeName/$portName]}"
    elif [ -n "$nodeName" ] && [ -n "${SINK_NICKNAMES[$nodeName]}" ]; then
        SINK_NICKNAME="${SINK_NICKNAMES[$nodeName]}"
        ...
        for glob in "${!SINK_NICKNAMES[@]}"; do
            ...
            if [[ "$nodeName/$portName" == $glob ]] || [[ "$nodeName" == $glob ]]; then
                ...
                SINK_NICKNAMES["$nodeName"]="$SINK_NICKNAME"
                break
            fi
        done
    fi
