...
case $desktop in
	...
	if [ "$m" == "DP-0" ]; then
          MONITOR=$m polybar --reload mainbar-bspwm-rosemary -c ~/.config/polybar/config-custom.ini &
    #     MONITOR=$m polybar --reload mainbar-bspwm -c ~/.config/polybar/config.ini &
	else
          MONITOR=$m polybar --reload mainbar-bspwm-rosemary-alt -c ~/.config/polybar/config-custom.ini &
	fi
      done
    else
