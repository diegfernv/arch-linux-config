...
case $desktop in
	...
	if [ "$m" == "DP-0" ]; then
          MONITOR=$m polybar --reload mainbar-bspwm-rosemary -c ~/.config/polybar/custom.old//config-custom.ini &
    #     MONITOR=$m polybar --reload mainbar-bspwm -c ~/.config/polybar/config.ini &
	else
          MONITOR=$m polybar --reload mainbar-bspwm-rosemary-alt -c ~/.config/polybar/custom.old//config-custom.ini &
	fi
      done
    else
    polybar --reload mainbar-bspwm -c ~/.config/polybar/custom.old/config.ini &
    fi
    # second polybar at bottom
    # if type "xrandr" > /dev/null; then
    #   for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
    #     MONITOR=$m polybar --reload mainbar-bspwm-extra-rosemary -c ~/.config/polybar/config.ini &
    #   done
    # else
